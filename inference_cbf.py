...
from models.content_based_cf import ContentBasedCF

# 定义数据路径
PROCESSED_DATA_PATH = "data/processed_data.pkl"
MODEL_PATH = "models/cbr_model.pkl"
RECOMMENDATIONS_PATH = "results/cbr_recommendations.txt"
...
def inference():
    ...
    for user_id in tqdm(test_data["user_id"].unique(), desc="Generating Recommendations"):
        try:
            recommendations[user_id] = cbr_model.recommend(user_id, top_k=10)
        except ValueError:
