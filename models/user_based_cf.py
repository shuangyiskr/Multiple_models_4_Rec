...
class UserBasedCF:
    ...
    def recommend(self, user_id, top_k=10):
        ...
        # 计算加权评分
        weighted_scores = self.user_similarity_matrix[user_id].values.reshape(-1, 1) * self.rating_matrix.values
        weighted_scores = pd.DataFrame(weighted_scores, index=self.rating_matrix.index, columns=self.rating_matrix.columns)
        
        # 过滤掉用户已经评分过的电影
        recommendations = weighted_scores.sum(axis=0) / (self.user_similarity_matrix[user_id].sum() + 1e-8)
